...
def exp_loss(pred, target_cls, time, toa, device, fps=20.0):
    ...
    coefficient_a = 0
    pos_loss = -torch.mul(torch.exp(coefficient_a*penalty), -ce_loss_fn(pred, target_cls))
    # negative example
...
def main():
    ...
    # Define network
    model =  Img_Graph_Multi_Graph_Net(input_dim=opt.input_dim, embedding_dim=opt.embedding_dim,
